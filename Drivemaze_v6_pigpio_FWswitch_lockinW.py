#drivemaze v6, pigpio lock in after F/W switch
#2022May
#8th
...
if trial_type == "Animal":  
    subjects = ["328340226232" ,"x", "y"] #out of study,  ""

#general variables
heavy=40 #heavy limit for weight based sensing
scale_cal=12026362.500 #use a calibration code and 20g weight to find out
nest_timeout=60000 #nest choice timeout in ms
lock_in_time=120000 #time locked into mismatched water chamber
...
class SaveData:
    ...
    def append_event(self,amount_consumed,latency_to_consumption,event_type,animaltag,frame,hardware_time):
        """
        Function used to save event parameters to a .csv file
        example use save.append_event("", "", "initialize", animaltag,frame_counter,tick)
        """
        global event_list
...
while True:
    ...
    if mode==0: #SEM open for entry
        open_door(1)
        close_door(2)
        if not pi.read(beaml[0]) and pi.read(beaml[1]):
            mode=1
            print("m1")
            w=int(10)
...
#             animaltag=tag
        if tag==335490249236 or tag==328340226232 or tag==328340178184:
            ...
            frame_counter=fc.tally()
            tick = pi.get_current_tick()
            save.append_event("", "", "leave_nest", animaltag,frame_counter,tick)
            #print("m2")
    if mode==2:
#         pi.write(start_scope_port,1) #start acquisition      
    ...
    if mode==3: #SEM open to maze
        ...
        frame_counter=fc.tally()
        tick = pi.get_current_tick()
        save.append_event("", "", "block_start", animaltag,frame_counter,tick)
        frame_counter=fc.tally()
        tick = pi.get_current_tick()
        save.append_event("", "", "imaging_start", animaltag,frame_counter,tick)
    if mode==4: #maze operational
        if not maze_entry_flag:
            if not pi.read(beaml[2]) or not pi.read(beaml[4]) or not pi.read(beaml[6]) or not pi.read(beaml[8]) or not pi.read(beaml[10]) :# or ...: #enter any unit
                ...
                #print("in maze")
        if not pi.read(beaml[2]) and not pod_entry_flag: #enter unit1
            frame_counter=fc.tally()
            tick = pi.get_current_tick()
            save.append_event("", "", "enter_explore", animaltag,frame_counter,tick)
            pod_entry_flag=True
            #print("unit1")
            close_door(3)
            open_door(4)
        ...
        if rec_wheel_flag:
            ...
            if save_flag and counter>limit/4: #detect latency to run start at first quarter revolution
                frame_counter=fc.tally()
                tick = pi.get_current_tick()
                save.append_event("", "", "run", animaltag,frame_counter,tick) 
        ...
        if not pi.read(beaml[5]) and pod_entry_flag: #exit
            ...
            rec_wheel_flag=False
            wheel_flag=False
        if not pi.read(beaml[10]) and not pod_entry_flag: #enter unit5 FOOD HERE NOW !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
            frame_counter=fc.tally()
            tick = pi.get_current_tick()
        ...
        if not pi.read(beaml[9]) and not pod_entry_flag: #enter unit4            
            frame_counter=fc.tally()
            tick = pi.get_current_tick()
            save.append_event("", "", "enter_social", animaltag,frame_counter,tick)
            pod_entry_flag=True
            #print("unit4")
            close_door(9)
            open_door(10)
        ...
        if not pi.read(beaml[8]) and pod_entry_flag: #exit
            ...
            open_door(9)
            social_flag=False
        if not pi.read(beaml[6]) and not pod_entry_flag: #enter unit3 WATER HERE NOW!!!!!!!!!!!!!!!!!!!!!! LOCK IN WHEN ENTERING!!!!! 
            frame_counter=fc.tally()
            tick = pi.get_current_tick()
            save.append_event("", "", "enter_drink", animaltag,frame_counter,tick)
            pod_entry_flag=True
            #print("unit3")
            close_door(7)
            close_door(8) #close 8 to lock in!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        ...
        if rec_licks_flag:
            ...
            if pi.read(lick_in_port):
                ...
                if water_flag:
                    frame_counter=fc.tally()
                    tick = pi.get_current_tick()
                    save.append_event("", "", "drink", animaltag,frame_counter,tick) 
            ...
            if water_stop_dose_flag and millis-water_dose_timer>water_duration:
                ...
                #print(drink_delay)    
            if millis-lick_timer>lock_in_time:
                open_door(8)               
        if not pi.read(beaml[7]) and pod_entry_flag: #exit
            frame_counter=fc.tally()
            tick = pi.get_current_tick()
            save.append_event(licks, drink_delay, "exit_drink", animaltag,frame_counter,tick)
            pod_entry_flag=False
            #print("ex3")
            close_door(8)
            open_door(7)
        ...
        if not pi.read(beaml[0]) and maze_entry_flag and not sense_SEM_flag: #putative leave maze
        ...
        if sense_SEM_flag and millis-scale_timer>scale_interval_scan:
            ...
            if w>15: #definite leave maze
                frame_counter=fc.tally()
                tick = pi.get_current_tick()
                save.append_event("", "", "block_end", animaltag,frame_counter,tick)
                close_door(2)
                open_door(1)
                mode=5
                #print("m5")
    ...
    if mode==5: #wait for exit or entry of other
        ...
        if w<3 and pi.read(beaml[1]):
            ...
            w = scale.getWeight() * 1000
            if not exit_safety_timer_flag and w<3 and pi.read(beaml[1]) and millis-exit_safety_timer>safety_delay:
                close_door(1)
    ...
    if mode==6: #wait for nest choice time out
        if millis-acquisition_end_timer>acquisition_end_timeout and not stop_imaging_flag:
            ...
            frame_counter=fc.tally()
            tick = pi.get_current_tick()
            save.append_event("", "", "imaging_stop", animaltag,frame_counter,tick)
        ...
        if millis-nest_timer>nest_timeout:
            ...
            for x in range(np.size(subjects)):
                animaltag=subjects[x]
                save.append_event("", "", "block_available", animaltag,frame_counter,tick)
            mode=0
            #print("m0") 
