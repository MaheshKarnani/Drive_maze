...
slowness=200 #milliseconds to wait between alternating doorset servo strokes
...
#             target[doorl[7]] = closel[doorl[7]]   
    ...
    if max(abs(d_state))>2:                 #if doors are not at target
        x=x+1
        if x==12:   #cycle through doors
            x=0
        if abs(d_state[x])>2: #find door not at target
            millis = int(round(time.time() * 1000))
...
#                 if abs(d_state[x])>2:       #
                if d_stroke==1:         #if it's this stroke direction's turn
                    ...
                    d_stroke=1
                kit.servo[x].angle = current[x]             #movement                    
                door_timer=millis
