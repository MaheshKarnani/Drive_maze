...
import smbus2
import yagmail
experiment ="baseline"#"FEDoff"#"baseline"#"FED removed""FWswap"
FEDonFLAG = True #change for turning FED on or off
Swap_Flag = False # change for swapping food and water pod
pi = pigpio.pi() 
...
#             print("no movement")
        ...
        if not pi.read(beaml[2]) and not pod_entry_flag and not Swap_Flag: #enter unit1 FOOD
            email_timer = int(round(time.time() * 1000))
            frame_counter=fc.tally()
            tick = pi.get_current_tick()
        ...
        if not pi.read(beaml[2]) and not pod_entry_flag and Swap_Flag: #uni1 is water
            email_timer = int(round(time.time() * 1000))
            frame_counter=fc.tally()
            tick = pi.get_current_tick()
        ...
        if not pi.read(beaml[3]) and pod_entry_flag and not Swap_Flag: #exit food
            email_timer = int(round(time.time() * 1000))
            frame_counter=fc.tally()
            tick = pi.get_current_tick()
        ...
        if not pi.read(beaml[3]) and pod_entry_flag and Swap_Flag: #exit unit 1 water       
            email_timer = int(round(time.time() * 1000))
            pi.write(water_reset_port,0)
            frame_counter=fc.tally()
            tick = pi.get_current_tick()
        ...
        if not pi.read(beaml[9]) and pod_entry_flag: #exit
            ...
            open_door(14)#open social door
            social_flag=False
        if not pi.read(beaml[10]) and not pod_entry_flag and not Swap_Flag: #enter unit5
            email_timer = int(round(time.time() * 1000))
            frame_counter=fc.tally()
            tick = pi.get_current_tick()
            save.append_event("", "", "enter_drink", animaltag,frame_counter,tick,experiment)
            ...
            licks=0
            drink_delay=[]
        if not pi.read(beaml[10]) and not pod_entry_flag and Swap_Flag: #unit 5 is food
            email_timer = int(round(time.time() * 1000))
            frame_counter=fc.tally()
            tick = pi.get_current_tick()
            save.append_event("", "", "enter_feed", animaltag,frame_counter,tick,experiment)
        ...
        if rec_licks_flag:
            ...
            if water_stop_dose_flag and millis-water_dose_timer>water_duration:
                ...
                #print(drink_delay)                   
        if not pi.read(beaml[11]) and pod_entry_flag and not Swap_Flag: #exit
            email_timer = int(round(time.time() * 1000))
            #pi.write(water_reset_port,0)
            frame_counter=fc.tally()
            tick = pi.get_current_tick()
            ...
            #pi.write(water_reset_port,1)
            rec_licks_flag=False
        if not pi.read(beaml[11]) and pod_entry_flag and Swap_Flag:   #exit uit 5 is exit food
            email_timer = int(round(time.time() * 1000))
            frame_counter=fc.tally()
            tick = pi.get_current_tick()
            save.append_event(pellets, food_delay, "exit_feed", animaltag,frame_counter,tick,experiment)
        ...
        if sense_SEM_flag and millis-scale_timer>scale_interval_scan:
            ...
            scale_timer=int(round(time.time() * 1000))
            if w>10 and not pi.read(beaml[0]): #definite leave maze
                frame_counter=fc.tally()
                tick = pi.get_current_tick()
